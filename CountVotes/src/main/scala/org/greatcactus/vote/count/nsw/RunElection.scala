...
object RunElection extends App {
  ...
  worker.run()
  ElectionReport.saveReports(new java.io.File(NSWStateElectionData2019.reportDir,"Single Run"+(if (useIvote) " iVote" else "")+(if (useNormal) " Normal" else "")),worker.report,rawstats.meta)
  //println("\n\nIVOTE\n")
 ...
 //        println("totalRemainingVotes = "+totalRemainingVotes+" undistributedVotes="+undistributedVotes+" continuingCandidates="+continuingCandidates+" tallys="+continuingCandidates.toList.map{tallys(_).numVotes})
    ...
    def distributeOneCandidatesWorthOfUndistributedVotes() {
     ...
     // count+=1
      ...
      val transferValueDescription = new TransferValueComputationSingleValue(candidateToDistribute.surplusVotes,numVotesToBeRedistributed,numRedistributedExhausted,transferValue)
      ...
      // Step 21 
      report.declareCandidateDistributed(candidateToDistribute.candidateID,candidateToDistribute.surplusVotes,totalTransferred,transferValueDescription,sortedRecipients,numVotesToBeRedistributed,numExhaustedSetAside,numExhaustedThatWouldBeCarriedOn,true)

...
object FederalElectionData {
  ...
  def load2013(state:String) = {
    val cache = new File("Cache/Federal/2013/"+state+".txt")
    try {
  ...
  def loadRaw2013(state:String) = {
    val sourcedir = new File("Elections/Federal/2013")
    def rel(f:String) = new File(sourcedir,f)
    loadRaw(state,new AECCandidateInformationSource2013GroupTickets(rel("SenateGroupVotingTicketsDownload-17496.csv")),new AECVoteSource2013ATL(rel("SenateUseOfGvtByGroupDownload-17496.csv")),new AECVoteSource2013BTL(rel("SenateStateBtlDownload-17496-"+state+".zip")))
  }
  def load2016(state:String) = {
    val cache = new File("Cache/Federal/2016/"+state+".txt")
    try {
      ElectionDataFastIO.loadPickled(cache)
    } catch { 
      case e:Exception => 
  ...
  def loadRaw2016(state:String) = {
    ...
    def rel(f:String) = new File(sourcedir,f)
    loadRaw(state,new AECCandidateInformationSource2016FirstPrefsByStateByVoteType(rel("SenateFirstPrefsByStateByVoteTypeDownload-20499.csv")),new AECVoteSource2016(rel("aec-senate-formalpreferences-20499-"+state+".zip")))
  }
  ...
  def loadRaw(state:String,candidatesSource:AECCandidateInformationSource,voteSources:AECVoteSource*) : ElectionData = {
    ...
    helper.getData(candidatesInfo.orderedCandidates,state) 
  }

  def readOfficialResults2013(data:ElectionData) : AECDistributionOfPreferencesOfficial = {
    val zipfile = new File("Elections/Federal/2013/SenateDopDownload-17496.zip")
...
class AECCandidateInformationSource2013GroupTickets(val groupVotingTickets:File) extends AECCandidateInformationSource {
  override def read(state:String) {
        for (cols<-CSVHelper(groupVotingTickets,2)) if (state==cols(0)) {
          ...
          val ticket = groupTickets.getOrElseUpdate(ownerTicket,{new GroupTicketBuilder(ownerTicket, cols(2))}) 
          val candidateID = cols(5)
          val candidate = candidates.getOrElseUpdate(candidateID,{
            val group = cols(6)
            groupTickets.getOrElseUpdate(group,{new GroupTicketBuilder(group, cols(11))}) 
            val surname = cols(7)
            val givenname = cols(8)
            val position = cols(9)
            val name = givenname+" "+surname
...
class AECCandidateInformationSource2016FirstPrefsByStateByVoteType(val firstPrefsByStateByVoteType:File) extends AECCandidateInformationSource {
  ...
  override def read(state:String) {
      for (cols<-CSVHelper(firstPrefsByStateByVoteType,2)) if (state==cols(0)) {
        ...
        if (candidateID!="0") {
          val ticket = groupTickets.getOrElseUpdate(groupID,{new GroupTicketBuilder(groupID, cols(5))}) 
...
class GroupTicketBuilder(val groupId:String,val groupName:String) {
  ...
  def toGroupInfo(candidateToInt:Map[Candidate,Int]) : GroupInformation = {
    ...
    new GroupInformation(groupId,groupName,tickets)
  }
}


object CSVHelper {
  def fromIS(stream:InputStream,headingLines:Int,separator:Char=',') = new CSVHelper(stream,headingLines,separator)
  def apply(file:File,headingLines:Int,separator:Char=',') = new CSVHelper(new FileInputStream(file),headingLines,separator)
  def fromZipFile(zipedFile:File,innerfilename:String,headingLines:Int) = {
    val zipFile = new ZipFile(zipedFile)
    import collection.JavaConversions._
    zipFile.entries().toList.find { _.getName.contains(innerfilename) } match {
...
class CSVHelper(stream:InputStream,headingLines:Int,separator:Char=',') {

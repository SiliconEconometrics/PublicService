...
//            println(" Ward "+ward.id+" available "+ward.isDataAvailable+" isWard="+isWard)
            def parseResult(result:Document) = {
              val winners = result.select("span.candidate-name")
...
//              for (name<-names) println("     "+name)
            ...
            if (ward.isDataAvailable) {              
              if (ward.isMayoral) {
                ...
                val winners2 = getAndRunJsoup(wardUrlBase+"candidates_in_sequence.html",{doc=>{doc.select("td[headers=1]").toList.map{_.text()}.toArray}})
...
//                if (winners2.isDefined) { println("winners2 : "); winners2.get.foreach { println _ } }
                if (winners.isDefined && winners.get.length>0) {
                  ...
                  if (dop1.isDefined) {
                    ...
                    val dop = getAndRunJsoup(wardUrlBase+"dop_index.html",{doc=>{(0::(doc.select("td a").toList.map{_.text().toInt})).max}})
...
class RunLotsOfContests(val year:Int) {
    ...
    def runElection(data:ElectionData,electedCandidates:List[String],isMayoral:Boolean,mayor:Option[String],officialDOP : Option[NSWWholeCountSummary],year:Int) {
                       ...
                       if (NSWLocalData.giveDetailedReportOnOneRun) {
                          val worker = new NSWElectionHelper(data,electedCandidates.length,new scala.util.Random,NSWLocalData.electionRules,ineligibleCandidates)
                          worker.run()
                       ...
                       val report = if (NSWLocalData.generateStochasticReports) Some(new StochasticReportOptions(new File("LGE"+year+"reports/"+prettyName+"/Stochastic"),false)) else None
                       ProbabilisticWork.runProbabilisticly(NSWLocalData.numToRun, NSWLocalData.numThreads, data,true,electedCandidates.length,NSWLocalData.electionRules,Some(output),ineligibleCandidates,report,officialDOP)
...
object NSWLocalData {
  ...
  val restrictCalculatedTo : Option[Set[String]] = None // Some(Set("Griffith City Council")) // Some(Set("Griffith"))
